...
class JsonBinaryDecoderService
{
     ...
     * @throws EventBinaryDataException
     */
    private function parseJson(int $type)
    {
        if (self::SMALL_OBJECT === $type) {
            $this->parseObject(EventBinaryData::UNSIGNED_SHORT_LENGTH);
        } else if (self::LARGE_OBJECT === $type) {
            $this->parseObject(EventBinaryData::UNSIGNED_INT32_LENGTH);
        } else if (self::SMALL_ARRAY === $type) {
            $this->parseArray(EventBinaryData::UNSIGNED_SHORT_LENGTH);
     ...
     * @throws EventBinaryDataException
     */
    private function parseObject(int $intSize)
    {
    ...
    private function parseValueType(int $numBytes, int $intSize): JsonBinaryDecoderValue
    {
        ...
        if (EventBinaryData::UNSIGNED_INT32_LENGTH === $intSize) {
            if (self::INT32 === $type) {
                return new JsonBinaryDecoderValue(
                    true,
    ...
    {
        $literal = ord($this->binaryDataReader->read(EventBinaryData::UNSIGNED_SHORT_LENGTH));
        if (0 === $literal) {
            return null;
        }
        if (1 === $literal) {
     ...
     * @throws JsonBinaryDecoderException
     */
    private function parseArray(int $size)
    {
     ...
     * @throws JsonBinaryDecoderException
     */
    private function parseScalar(int $type)
    {
        if (self::LITERAL === $type) {
            $this->parseBoolean();
