...
class ColumnDTO
{
    ...
    public static function make(
        int $columnType,
        FieldDTO $fieldDTO,
        EventBinaryData $binaryDataReader
    ): self {
        $maxLength = 0;
        $size = 0;
        $fsp = 0;
        ...
        if ($columnType === FieldTypeConst::VARCHAR) {
            $maxLength = $binaryDataReader->readInt16();
        } else if ($columnType === FieldTypeConst::DOUBLE) {
            $size = $binaryDataReader->readUInt8();
        } else if ($columnType === FieldTypeConst::FLOAT) {
            $size = $binaryDataReader->readUInt8();
        } else if ($columnType === FieldTypeConst::TIMESTAMP2) {
            $fsp = $binaryDataReader->readUInt8();
        } else if ($columnType === FieldTypeConst::DATETIME2) {
            $fsp = $binaryDataReader->readUInt8();
        } else if ($columnType === FieldTypeConst::TIME2) {
            $fsp = $binaryDataReader->readUInt8();
        } else if ($columnType === FieldTypeConst::VAR_STRING || $columnType === FieldTypeConst::STRING) {
            $metadata = ($binaryDataReader->readUInt8() << 8) + $binaryDataReader->readUInt8();
            $realType = $metadata >> 8;
            if ($realType === FieldTypeConst::SET || $realType === FieldTypeConst::ENUM) {
                $columnType = $realType;
                $size = $metadata & 0x00ff;
            } else {
                $maxLength = ((($metadata >> 4) & 0x300) ^ 0x300) + ($metadata & 0x00ff);
            }
        } else if ($columnType === FieldTypeConst::BLOB || $columnType === FieldTypeConst::IGNORE) {
            $lengthSize = $binaryDataReader->readUInt8();
        } else if ($columnType === FieldTypeConst::GEOMETRY) {
            $lengthSize = $binaryDataReader->readUInt8();
        } else if ($columnType === FieldTypeConst::JSON) {
            $lengthSize = $binaryDataReader->readUInt8();
        } else if ($columnType === FieldTypeConst::NEWDECIMAL) {
            $precision = $binaryDataReader->readUInt8();
            $decimals = $binaryDataReader->readUInt8();
        } else if ($columnType === FieldTypeConst::BIT) {
    ...
    {
        if ($this->type === FieldTypeConst::ENUM) {
    ...
    {
        if ($this->type === FieldTypeConst::SET) {
